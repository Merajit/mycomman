...
AUTH0_DOMAIN = 'dev-07gnmg2g.us.auth0.com'
API_AUDIENCE = "https://dev-07gnmg2g.us.auth0.com/api/v2/"
ALGORITHMS = ["RS256"]
...
def private_scoped():
    ...
    if requires_scope("read:users"):
        response = "Hello from a private-scoped endpoint! You need to be authenticated and have a scope of read:messages to see this."
        return jsonify(message=response)
    raise AuthError({
        "code": "Unauthorized",
...
def create_user():
    if requires_scope("create:users"):
        response = "Hello from a create endpoint! You need to be authenticated and have a scope of read:messages to see this."
        return jsonify(message=response)
    raise AuthError({
        "code": "Unauthorized",
        "description": "You don't have access to this resource"
